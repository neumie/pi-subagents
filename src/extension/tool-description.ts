...
• Keep execution and management separate: omit action for direct or workflowScript execution; use action only for management/control.
...
export const FULL_SUBAGENT_TOOL_DESCRIPTION = `Delegate one child with { agent, task } or compose work with { workflowScript }; omit action. Use action only for management/control actions.

EXECUTION:
• Before executing, use { action: "list" } and run only executable/non-disabled configured agents.
• SINGLE: { agent: "worker", task: "..." } launches one child. Omit task only for a self-contained agent.
• COMPATIBILITY: Top-level { tasks: [...] } and { chain: [...] } remain accepted; chain may pair with top-level task as its original template input. Prefer workflowScript for new multi-child orchestration.
...
export const COMPACT_SUBAGENT_TOOL_DESCRIPTION = `Delegate one child with { agent, task } or compose work with { workflowScript }; omit action. Use action only for management/control actions.

EXECUTE:
• Call { action:"list" } first and use only executable/non-disabled agents.
• SINGLE {agent:"worker",task:"..."} launches one child.
