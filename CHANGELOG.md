...
### Added
- Added `/subagents-watch`, a read-only current-session async child watcher with bounded task previews and sanitized transcript tails.
...
### Fixed
- Default Pi Lens child startup to quick mode without delayed warmup while preserving explicit Lens environment overrides.
