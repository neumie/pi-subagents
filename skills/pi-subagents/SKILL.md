...
Use this skill when the parent orchestrator needs one specialized child or composed orchestration. Delegate one isolated child directly with `{ agent, task }`; use `workflowScript` for coordinated waves: sequence, parallelism, branching, retries, gate monitors, and aggregation. Bounded top-level `tasks` and `chain` inputs remain accepted for compatibility, but prefer `workflowScript` for new composed work. Scripted workflows start asynchronously by default; pass `async:false` only for a small foreground run.
