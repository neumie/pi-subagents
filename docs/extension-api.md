...
- `spawn` accepts direct `{ agent, task }`, bounded top-level `tasks`/`chain` compatibility, or `workflowScript`, and is async-only: omit `async` or set `async: true`, omit `clarify` or set it to `false`, and do not pass management `action` values. It goes through the same executor as the `subagent` tool, so agent discovery, validation, session attribution, configured spawn caps, child-safety depth, artifacts, and async status all behave the same.
